import { DesignItem, hideAtRunTimeAttributeName } from '@node-projects/web-component-designer';
import { attributesForBarcode, barcodeCommandRegistry, getBarcodeFieldOriginOffset, readBarcodeProps } from '../barcodes/barcodeRegistry.js';
...
const first = (...values) => values.find(value => value !== '' && value != null && value !== 'NaN');
/** Zebra uses zero/omitted width as the natural aspect ratio. */
export const parseZplFontWidth = (value) => Number(first(value, 0));
function setPosition(element, x, y) {
    element.style.position = 'absolute';
    element.style.left = `${Number.isFinite(x) ? x : 0}px`;
    element.style.top = `${Number.isFinite(y) ? y : 0}px`;
}
function designItem(element, serviceContainer, instanceServiceContainer) {
...
export class ZplParserService {
    options = {};
    async parse(source, serviceContainer, instanceServiceContainer, _parseSnippet) {
        return this._parse(source, serviceContainer, instanceServiceContainer, true);
    }
    async _parse(source, serviceContainer, instanceServiceContainer, includeHidden) {
        ...
        let fontWidth = 0;
        let fontRotation = 'N';
        let by = { moduleWidth: 2, wideRatio: 3, barHeight: 100 };
        ...
        for (const token of tokens) {
            ...
            if (token.command === 'FO') {
                ...
                if (pendingBarcode) {
                    ...
                    const offset = getZplTextOutputOffset(fontRotation);
        ...
        for (const payload of hidden) {
            ...
            for (const item of hiddenItems)
                item._withoutUndoSetAttribute(hideAtRunTimeAttributeName, '');
            result.splice(Math.min(payload.index, result.length), 0, ...hiddenItems);
        }
        return result;
    }
    write(textWriter, designItems, _rootContainerKeepInline, updatePositions) {
        ...
        for (let index = 0; index < designItems.length; index++) {
            ...
            if (designItem.hideAtRunTime) {
                const miniLines = ['^XA'];
                if (designItem.element instanceof ZplImage)
                    miniLines.push(designItem.element.createZplImage());
                const createZpl = designItem.element.createZpl;
                if (createZpl)
                    miniLines.push(createZpl.call(designItem.element));
                miniLines.push('^XZ');
                ...
                const createZpl = designItem.element.createZpl;
                if (createZpl)
                    textWriter.writeLine(createZpl.call(designItem.element));
            }
