...
function findCommandStart(source, from, includeTilde = true) {
    for (let index = from; index + 2 < source.length; index++) {
        const prefix = source[index];
        if (prefix !== '^' && (!includeTilde || prefix !== '~'))
            continue;
        if (/^[A-Za-z0-9]{2}$/.test(source.slice(index + 1, index + 3)))
            return index;
    }
    return -1;
}
export function tokenizeZpl(source) {
    const tokens = [];
    let index = 0;
    while (index < source.length) {
        const start = findCommandStart(source, index);
        if (start < 0 || start + 2 >= source.length)
            break;
...
export function createHiddenZplComments(index, miniZpl) {
...
export function hiddenPayloads(tokens) {
    const regex = /^NODEPROJECTS-HIDDEN-V1:(\d+):(\d+)\/(\d+):([A-Za-z0-9_-]+)$/;
    const groups = new Map();
    for (const token of tokens) {
        if (token.prefix !== '^' || token.command !== 'FX')
            continue;
        ...
        if (!Number.isInteger(index) || part < 1 || total < 1 || part > total)
            continue;
    ...
    for (const [index, group] of groups) {
        if (group.invalid || group.parts.size !== group.total)
            continue;
        try {
...
export function decodeHiddenZplComments(source) {
