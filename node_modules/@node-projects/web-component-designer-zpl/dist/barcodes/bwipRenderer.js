...
import { getBarcodeDefinition, getDataMatrixVersion, isDataMatrixRectangular } from './barcodeRegistry.js';
export const eanUpcTypes = new Set(['ean13', 'ean8', 'upca', 'upce']);
...
const tlc39Rows = [6, 8, 10, 12, 15, 20, 26, 32, 38, 44];
const tlc39LinkGapModules = 10;
function microPdf417Mode(mode) {
    return microPdf417Modes[mode] ?? microPdf417Modes[0];
}
function estimatePdf417Columns(content, securityLevel) {
...
export function barcodeShowsInterpretation(props) {
...
export function barcodeHorizontalInsets(props) {
...
export function getEanUpcHriFragments(props) {
    return rawEanUpc(props)?.txt?.map(fragment => ({ char: fragment[0], xModule: fragment[1] })) ?? [];
}
const eanUpcFontDots = [8, 18, 28, 28, 28, 28, 28, 56, 56, 56];
const eanUpcGapDots = [4, 4, 5, 5, 5, 5, 5, 7, 7, 7];
/** EAN/UPC HRI uses the same discrete firmware font sizes as ZPLab/Labelary. */
export function getEanUpcHriStyle(moduleWidth) {
...
function bwipRetryOptions(options) {
...
export function buildBwipOptions(props) {
    const definition = getBarcodeDefinition(props.type);
    if (definition.bwipOptions)
        return definition.bwipOptions(props);
    if (definition.resizeKind === 'linear') {
        const v = props;
        let bcid = definition.bcid;
        let text = v.content || definition.defaultContent || '0';
        if (v.type === 'upceanextension')
            bcid = text.length === 2 ? 'ean2' : 'ean5';
        if (v.type === 'upce' && text.length === 6)
            text = `0${text}`;
        if (v.type === 'logmars' || v.type === 'code39' || v.type === 'codabar' || v.type === 'plessey')
            text = text.toUpperCase();
        if (v.type === 'code11')
            text += code11CheckDigits(text, !v.checkDigit);
        if (v.type === 'planet') {
        ...
        if (v.checkDigit && ['code39', 'interleaved2of5', 'msi'].includes(v.type))
    ...
    switch (props.type) {
        ...
        case 'aztec': {
            const v = props;
            const options = { bcid: 'azteccodecompact', text: v.content || ' ', scale: v.magnification };
            if (v.ecLevel === 300)
                Object.assign(options, { bcid: 'azteccode', format: 'rune' });
            else if (v.ecLevel >= 201 && v.ecLevel <= 232)
                Object.assign(options, { bcid: 'azteccode', format: 'full', layers: v.ecLevel - 200 });
            else if (v.ecLevel >= 101 && v.ecLevel <= 104)
                options.layers = v.ecLevel - 100;
            else if (v.ecLevel >= 5 && v.ecLevel <= 95)
                options.eclevel = v.ecLevel;
            return options;
        }
        case 'maxicode': {
            const v = props;
...
export function measureBarcodeModules(props) {
    ...
    if (!symbol)
        throw new Error('BWIP-JS returned no geometry');
    if (symbol.pixx != null && symbol.pixy != null)
        return { width: symbol.pixx, height: symbol.pixy };
 ...
 */
export function measureBarcodePreview(props, fallback) {
    switch (props.type) {
        case 'qrcode': {
            const modules = measureBarcodeModules(props);
            const size = props.magnification;
...
export function getZebraWidthBarGeometry(props) {
    if (!['plessey', 'planet', 'postnet'].includes(props.type))
        return null;
    ...
    if (props.type === 'plessey' && raw.sbs) {
        ...
        if (runs.some(value => value === 0))
            throw new Error('BWIP-JS returned unsupported Plessey geometry');
        runs.forEach((modules, index) => {
            const runWidth = modules * moduleWidth;
            if (index % 2 === 0)
                rects.push({ x: width, y: 0, width: runWidth, height });
            width += runWidth;
        });
    }
    else if (raw.bhs) {
...
function tlc39Code39Runs(text, wideRatio) {
    const engine = bwipjs;
    const raw = engine.raw({ bcid: 'code39', text: text || ' ' })[0];
    if (!raw?.sbs?.length)
        throw new Error('BWIP-JS returned no TLC39 Code 39 geometry');
    const runs = raw.sbs.map((value, index) => index % 10 === 9 ? 1 : Number(value) >= 3 ? wideRatio : 1);
    if (runs.length % 10 === 0)
        runs.pop();
    return runs;
}
function firstTlc39MicroPdf(serial) {
    const engine = bwipjs;
    for (const rows of tlc39Rows) {
        try {
            ...
            if (viewBox)
                return { rows, width: Number(viewBox[1]), height: Number(viewBox[2]) };
        }
...
export function getTlc39Geometry(props) {
...
function renderTlc39Canvas(props) {
    ...
    geometry.runs.forEach((modules, index) => {
        if (index % 2 === 0) {
            ...
            context.fillRect(x0, tall ? geometry.code39Y - geometry.stopOverhang : geometry.code39Y, Math.max(1, x1 - x0), props.barHeight + (tall ? 2 * geometry.stopOverhang : 0));
        }
        xModules += modules;
    });
    if (geometry.microPdf) {
        ...
        try {
            ...
            context.drawImage(source, geometry.microPdf.x, geometry.microPdf.y, geometry.microPdf.width, geometry.microPdf.height);
        }
...
export function measureBarcodeBounds(props) {
    if (props.type === 'tlc39') {
        const geometry = getTlc39Geometry(props);
        return { width: geometry.width, height: geometry.height };
    }
    if ('moduleWidth' in props) {
        const zebra = getZebraWidthBarGeometry(props);
        if (zebra)
            return { width: zebra.width, height: zebra.height };
    }
    const options = buildBwipOptions(props);
    const engine = bwipjs;
    let svg;
    ...
    const viewBox = svg.match(/viewBox="0 0 ([\d.]+) ([\d.]+)"/);
    if (!viewBox)
        throw new Error('BWIP-JS returned SVG without viewBox geometry');
    return measureBarcodePreview(props, { width: Number(viewBox[1]), height: Number(viewBox[2]) });
}
function renderZebraWidthBars(props) {
...
function renderEanUpcBars(props) {
...
function cropMaxicodeCanvas(source) {
    if (source.width < 2 || source.height < 3)
        return source;
    const canvas = document.createElement('canvas');
...
export function renderBarcode(props) {
    try {
        const validationError = getBarcodeDefinition(props.type).validate(props);
        if (validationError)
            return { canvas: null, error: validationError, width: 0, height: 0 };
        let canvas = eanUpcTypes.has(props.type) ? renderEanUpcBars(props)
            : props.type === 'tlc39' ? renderTlc39Canvas(props)
                : 'moduleWidth' in props ? renderZebraWidthBars(props) : null;
        if (!canvas) {
        ...
        if (props.type === 'maxicode')
            canvas = cropMaxicodeCanvas(canvas);
